...
def main() -> None:
    ...
    pkt = m.build_visit_packet(formats=["markdown"])
    check("build_visit_packet questions", pkt.get("questions_count", 0) >= 1)
    check("build_visit_packet disclaimer", bool(pkt.get("disclaimer")))
    check("build_visit_packet summary", len(pkt.get("summary_md", "")) > 100)
    
    hc = m.health_check(include_sources=False)
    check("health_check not expired", not hc.get("expired"))
    ...
    check("azure services map", set(contract.get("azure_services", {}).keys()) >= {"storage", "ai", "functions"})
    ...
    check("azure share stub", share and ("upload_token_preview" in share or share.get("share_url")))
    
    ds = m.list_data_sources()
    ids = [s.get("id") for s in (ds.get("sources") or ds.get("catalog") or [])]
    if not ids and ds.get("catalog"):
        ids = [s["id"] for s in ds["catalog"]]
    check("google_health in catalog", "google_health" in ids or any(s.get("id") == "google_health" for s in ds.get("catalog", [])))
    ...
    try:
        ...
        assert spec_api.loader is not None
